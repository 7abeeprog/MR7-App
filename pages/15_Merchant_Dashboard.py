...
# --- 3. واجهة React للوحة التاجر السيادي ---
react_html = """
<!DOCTYPE html>
<html dir="rtl">
<head>
    ...
    <style>
        ...
        .tab-active {
            ...
            border-right: 4px solid var(--mr7-gold);
            color: var(--mr7-gold) !important;
        }
...
<body>
    ...
    <script type="text/babel">
        ...
        const App = () => {
            ...
            const themes = {
                "غامق إمبراطوري 🖤": { bg: "bg-[#030303]", text: "text-white", card: "bg-[rgba(15,15,15,0.8)]", border: "border-[#FFD700]/20" },
                "فاتح ملكي ✨": { bg: "bg-[#F5F5F5]", text: "text-[#1A1A1A]", card: "bg-white/90", border: "border-[#B8860B]/20" },
                "أزرق القيادة 💙": { bg: "bg-[#001529]", text: "text-white", card: "bg-[#001F3F]/80", border: "border-[#0074D9]/20" },
                "أخضر الاستدامة 💚": { bg: "bg-[#001A10]", text: "text-white", card: "bg-[#002B1B]/80", border: "border-[#00FF88]/20" }
            };
            ...
            useEffect(() => { lucide.createIcons(); }, [activeTab, toasts, products, team]);
            ...
            return (
                <div className={`min-h-screen ${activeTheme.bg} ${activeTheme.text} flex flex-col md:flex-row overflow-hidden`}>
                    {/* Sidebar */}
                    <div className={`w-full md:w-72 md:min-h-screen ${activeTheme.card} border-b md:border-b-0 md:border-l ${activeTheme.border} flex flex-col`}>
                        <div className="p-8 pb-4">
                            <div className="bg-yellow-500 p-3 rounded-2xl inline-block text-black mb-4 shadow-[0_0_15px_rgba(255,215,0,0.3)]">
                                <Icon name="Briefcase" size={28} />
                            </div>
                            <h1 className="text-2xl font-black uppercase tracking-widest text-yellow-500 mb-1">لوحة التاجر</h1>
                        ...
                        <div className="flex flex-row md:flex-col gap-2 p-4 md:p-6 overflow-x-auto no-scrollbar md:flex-1">
                            <button onClick={() => setActiveTab('overview')} className={`flex items-center gap-4 px-6 py-4 rounded-2xl font-bold transition-all text-right whitespace-nowrap ${activeTab === 'overview' ? 'tab-active' : 'text-gray-400 hover:bg-white/5 hover:text-white'}`}>
                                <Icon name="LayoutDashboard" size={20} /> نظرة عامة
                            </button>
                            <button onClick={() => setActiveTab('products')} className={`flex items-center gap-4 px-6 py-4 rounded-2xl font-bold transition-all text-right whitespace-nowrap ${activeTab === 'products' ? 'tab-active' : 'text-gray-400 hover:bg-white/5 hover:text-white'}`}>
                                <Icon name="Package" size={20} /> إدارة الأصول والمخزون
                            </button>
                            <button onClick={() => setActiveTab('team')} className={`flex items-center gap-4 px-6 py-4 rounded-2xl font-bold transition-all text-right whitespace-nowrap ${activeTab === 'team' ? 'tab-active' : 'text-gray-400 hover:bg-white/5 hover:text-white'}`}>
                                <Icon name="Users" size={20} /> جيش التاجر المساعد
                            </button>
                            <button onClick={() => setActiveTab('services')} className={`flex items-center gap-4 px-6 py-4 rounded-2xl font-bold transition-all text-right whitespace-nowrap ${activeTab === 'services' ? 'tab-active' : 'text-gray-400 hover:bg-white/5 hover:text-white'}`}>
                        ...
                        <div className="hidden md:block p-6 mt-auto">
                            <div className="bg-white/5 p-5 rounded-3xl border border-white/5 text-center">
                                <Icon name="ShieldCheck" size={24} className="text-[#00FF88] mx-auto mb-2" />
                                <span className="block text-xs font-black text-gray-400 uppercase">تاجر معتمد</span>
                                <span className="block text-[#00FF88] font-bold text-sm mt-1">الجيل الثالث</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex-1 p-4 md:p-10 h-screen overflow-y-auto no-scrollbar">
                        {activeTab === 'overview' && (
                            <div className="animate-view space-y-8 max-w-6xl mx-auto">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                                    <div className={`${activeTheme.card} p-6 rounded-[2rem] border ${activeTheme.border} relative overflow-hidden group`}>
                                        <div className="absolute -right-4 -top-4 opacity-5 text-yellow-500 group-hover:scale-110 transition-transform"><Icon name="TrendingUp" size={120} /></div>
                                        ...
                                        <span className="text-xs text-yellow-500 font-black mt-3 flex items-center gap-1"><Icon name="ArrowUpRight" size={14}/> +15.4% هذا الربع</span>
                                    </div>
                                    <div className={`${activeTheme.card} p-6 rounded-[2rem] border ${activeTheme.border} relative overflow-hidden group`}>
                                    ...
                                    <div className={`${activeTheme.card} p-6 rounded-[2rem] border ${activeTheme.border} relative overflow-hidden group`}>
                                        <div className="absolute -right-4 -top-4 opacity-5 text-blue-500 group-hover:scale-110 transition-transform"><Icon name="Eye" size={120} /></div>
                                    ...
                                    <div className={`${activeTheme.card} p-6 rounded-[2rem] border ${activeTheme.border} relative overflow-hidden group`}>
                                        <div className="absolute -right-4 -top-4 opacity-5 text-purple-500 group-hover:scale-110 transition-transform"><Icon name="Users" size={120} /></div>
                                ...
                                <div className={`${activeTheme.card} p-8 rounded-[2rem] border ${activeTheme.border}`}>
                                            ...
                                            <tbody>
                                                {products.map(p => (
                                                    <tr key={p.id} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                                                        ...
                                                        <td className="py-4">
                                                            <span className={`px-3 py-1 rounded-lg text-xs font-black ${p.status === 'نشط' ? 'bg-[#00FF88]/20 text-[#00FF88]' : 'bg-yellow-500/20 text-yellow-500'}`}>{p.status}</span>
                                                        </td>
                                                    </tr>
                                                ))}
                                            </tbody>
                        ...
                        {activeTab === 'products' && (
                            <div className="animate-view space-y-8 max-w-4xl mx-auto">
                                <h2 className="text-3xl font-black mb-8 border-b border-white/10 pb-6 flex items-center gap-4">
                                    <Icon name="PlusCircle" className="text-yellow-500" size={32} /> طرح أصل جديد للسوق
                                </h2>
                                
                                <form onSubmit={handleAddProduct} className={`${activeTheme.card} p-8 md:p-10 rounded-[3rem] border ${activeTheme.border} shadow-2xl`}>
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
                                        <div className="space-y-3">
                                            <label className="text-xs font-black text-gray-400 uppercase tracking-widest">اسم الأصل / المنتج</label>
                                            <input type="text" className="w-full premium-input rounded-2xl py-4 px-5 font-bold" value={newProd.name} onChange={e => setNewProd({...newProd, name: e.target.value})} placeholder="مثال: يخت قيادي فاخر..." />
                                        </div>
                                        <div className="space-y-3">
                                            <label className="text-xs font-black text-gray-400 uppercase tracking-widest">القيمة التسويقية ($)</label>
                                            <input type="number" className="w-full premium-input rounded-2xl py-4 px-5 font-bold" value={newProd.price} onChange={e => setNewProd({...newProd, price: e.target.value})} placeholder="0.00" />
                                        </div>
                                        <div className="space-y-3">
                                            <label className="text-xs font-black text-gray-400 uppercase tracking-widest">القسم الاستراتيجي</label>
                                            <select className="w-full premium-input rounded-2xl py-4 px-5 font-bold text-white bg-black appearance-none" value={newProd.category} onChange={e => setNewProd({...newProd, category: e.target.value})}>
                                        ...
                                        <div className="space-y-3">
                                            ...
                                            <select className="w-full premium-input rounded-2xl py-4 px-5 font-bold text-white bg-black appearance-none">
                                                <option>مصر</option>
                                                <option>ليبيا</option>
                                        ...
                                        <div className="md:col-span-2 space-y-3">
                                            ...
                                            <textarea className="w-full premium-input rounded-2xl py-4 px-5 font-bold min-h-[120px]" value={newProd.desc} onChange={e => setNewProd({...newProd, desc: e.target.value})} placeholder="اشرح القيمة المضافة لهذا الأصل ليجذب القادة للاستحواذ عليه..."></textarea>
                                        </div>
                                    </div>
                                    <button type="submit" className="w-full py-6 rounded-[2rem] font-black text-xl bg-yellow-500 text-black btn-hover-gold flex items-center justify-center gap-3">
                        ...
                        {activeTab === 'team' && (
                            <div className="animate-view space-y-8 max-w-6xl mx-auto">
                                <div className="flex justify-between items-center mb-8 border-b border-white/10 pb-6">
                                    <h2 className="text-3xl font-black flex items-center gap-4"><Icon name="Users" className="text-yellow-500" size={32} /> جيش التاجر المعاون</h2>
                                    <button onClick={() => showToast('تم إرسال دعوة انضمام!', 'success')} className="bg-white/10 hover:bg-yellow-500 hover:text-black px-6 py-3 rounded-xl font-bold transition-colors flex items-center gap-2">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                    {team.map(member => (
                                        <div key={member.id} className={`${activeTheme.card} p-8 rounded-[2.5rem] border ${activeTheme.border} relative overflow-hidden`}>
                                            ...
                                            <p className="text-gray-400 text-sm font-bold mb-6">{member.role}</p>
                        ...
                        {activeTab === 'services' && (
                            <div className="animate-view space-y-8 max-w-6xl mx-auto">
                                <h2 className="text-3xl font-black mb-8 border-b border-white/10 pb-6 flex items-center gap-4">
                                    <Icon name="Zap" className="text-yellow-500" size={32} /> الخدمات الذكية المعاونة
                                </h2>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                                    <div className={`${activeTheme.card} p-8 rounded-[2.5rem] border ${activeTheme.border} text-center flex flex-col items-center hover:border-[#00FF88] transition-colors`}>
                                    ...
                                    <div className={`${activeTheme.card} p-8 rounded-[2.5rem] border ${activeTheme.border} text-center flex flex-col items-center hover:border-yellow-500 transition-colors`}>
                                        <div className="bg-yellow-500/10 text-yellow-500 p-5 rounded-3xl mb-6"><Icon name="Megaphone" size={40} /></div>
                                        <h4 className="text-xl font-black mb-3">حملة ضخ إقليمية</h4>
                                        <p className="text-gray-400 text-sm mb-8 leading-relaxed">ترويج أصولك بقوة داخل شبكة القادة في إقليم معين (مصر، ليبيا، السودان) لضمان مبيعات فورية.</p>
                                        <button onClick={() => showToast('سيتم توجيهك لمدير الحملات الإقليمية')} className="mt-auto w-full py-4 bg-white/5 hover:bg-yellow-500 hover:text-black rounded-xl font-black transition-all">طلب حملة (مخصص)</button>
                                    </div>
                                    <div className={`${activeTheme.card} p-8 rounded-[2.5rem] border ${activeTheme.border} text-center flex flex-col items-center hover:border-blue-500 transition-colors`}>
                                        <div className="bg-blue-500/10 text-blue-500 p-5 rounded-3xl mb-6"><Icon name="FileSignature" size={40} /></div>
                                        ...
                                        <button onClick={() => showToast('تم ربط متجرك بالخدمة القانونية')} className="mt-auto w-full py-4 bg-white/5 hover:bg-blue-500 hover:text-white rounded-xl font-black transition-all">تفعيل التوثيق التلقائي</button>
                                    </div>
                                </div>
                            </div>
                        )}
